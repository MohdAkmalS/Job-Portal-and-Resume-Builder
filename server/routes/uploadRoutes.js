...
const upload = multer({
    storage,
    limits: { fileSize: 3 * 1024 * 1024 }, // 3MB limit (3MB * 1.33 = ~4MB Payload. Vercel max is 4.5MB)
    ...
    upload(req, res, function (err) {
        if (err instanceof multer.MulterError) {
            ...
            if (err.code === 'LIMIT_FILE_SIZE') {
                return res.status(400).json({ success: false, message: 'File too large. Max limit is 3MB.' });
            }
